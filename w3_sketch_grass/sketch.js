...
function draw() {
  ...
  for (x = 700; x < 700; x++) {
    console.log(x);
    for (y = 0; y < 485; y++) {
      var redIn = testImage.pixels[(y * +x) * 4 + 0];
      ...
      redOut = testImage.pixels[(y2 * 700 + x2) * 4 + 0];
      greenOut = testImage.pixels[(y2 * 700 + x2) * 4 + 1];
      blueOut = testImage.pixels[(y2 * 700 + x2) * 4 + 2];


      testImage.pixels[(y * 700 + x) * 4 + 0] = redOut;
      testImage.pixels[(y * 700 + x) * 4 + 1] = greenOut;
      testImage.pixels[(y * 700 + x) * 4 + 2] = blueOut;
    }
  }
  testImage.updatePixels();

  image(testImage, 0, 0, 700, 700);
